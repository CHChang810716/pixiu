...
namespace http {
namespace impl = boost::beast::http;

using impl::request;
using impl::basic_fields;
using impl::string_body;
using impl::error;
using impl::status;
using impl::field;
using impl::verb;
...
using impl::response;
using impl::file_body;
using impl::empty_body;

template<class... T>
auto async_read(T&&... args) {
    std::cout << "async_read" << std::endl;
    return impl::async_read(std::forward<T>(args)...);
}

template<class... T>
auto async_write(T&&... args) {
    std::cout << "async_write" << std::endl;
    return impl::async_write(std::forward<T>(args)...);
}
...
struct send_lambda
{
    ...
    operator()(http::message<isRequest, Body, Fields>&& msg) const
    {
        ...
        close_ = msg.need_eof();
        std::cout << "close_" << ": " << close_ << std::endl;
