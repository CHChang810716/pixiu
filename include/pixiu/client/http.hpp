...
struct http 
: public std::enable_shared_from_this<http<IOContextAR>>{
protected:
  static auto& logger() {
...
public:
  ...
  using this_t    = http<IOContextAR>;
  
  http(IOContextAR& ioc)
  : ioc_(ioc)
  {}
  http()
  : ioc_()
  ...
  async_read(
    ...
    boost::asio::spawn(ioc_, [
      ...
      for(auto& req_param : req_vec) {
        ...
        __http::async_write(socket, req, yield[ec]);
        if(ec) {
      ...
      for(auto&& rep : reps) {
        __http::async_read(socket, recv_buf, rep, yield[ec]);
      }
